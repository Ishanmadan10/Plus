...
# -----------------------------
# Update backlog BEFORE committing
# so the status change is included in the same PR
# -----------------------------
...
# -----------------------------
# Git ops — stage everything including backlog.json
# -----------------------------
...
# Stage Swift changes + backlog.json together
subprocess.run(["git", "add", "-A"], check=True)

# Check if there's actually anything to commit
diff_check = subprocess.run(["git", "diff", "--cached", "--quiet"], check=False)
if diff_check.returncode == 0:
    print("No actual changes detected — marking suggestion as skipped")
    # Revert backlog status back to skipped so agent moves on next run
    for s in backlog["suggestions"]:
        if s["id"] == suggestion_id:
            s["status"] = "skipped"
            s.pop("pr", None)
...
> ⚠️ This PR includes a `backlog.json` update marking `{suggestion_id}` as implemented.
> Merging this PR will automatically unblock the next suggestion for the code agent.
...
print(f"Merging this PR will mark '{suggestion_id}' as implemented and unblock the next suggestion")